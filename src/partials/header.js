...
function desktopNavItem(item, page) {
  ...
  return `<li class="relative" data-services-menu><div class="flex items-center gap-1"><a href="services.html"${current} class="${classes}">Servicios</a><button type="button" class="grid h-6 w-6 place-items-center rounded-full text-(--rose-dark) transition-colors hover:bg-(--beige-light)" aria-label="Abrir áreas de práctica" aria-expanded="false" aria-controls="services-menu" data-services-toggle><svg class="h-3 w-3" width="1em" height="1em" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="m5 7 5 5 5-5"/></svg></button></div><nav id="services-menu" class="services-menu-panel absolute left-1/2 top-full z-10 mt-4 w-72 -translate-x-1/2 rounded-2xl border border-white/70 bg-white/95 p-3 shadow-[0_18px_44px_-24px_rgba(58,50,53,0.5)] backdrop-blur-md" aria-label="Áreas de práctica" hidden data-services-panel><p class="px-3 pb-2 pt-1 text-[10px] font-medium uppercase tracking-[0.2em] text-(--copper)">Áreas de práctica</p><ul class="grid gap-1">${serviceLinks('block rounded-xl px-3 py-2 text-[13px] text-(--charcoal) transition-colors hover:bg-(--beige-light) focus-visible:bg-(--beige-light)')}</ul></nav></li>`;
}
...
<nav id="mobile-menu" class="mobile-menu fixed inset-x-4 top-20 z-40 mx-auto max-h-[calc(100dvh-6rem)] max-w-340 overflow-y-auto rounded-lg border border-white/50 bg-white/90 p-4 shadow-[0_18px_44px_-24px_rgba(58,50,53,0.5)] backdrop-blur-md sm:inset-x-6 sm:top-24 xl:hidden" aria-label="Menú principal" data-mobile-menu><div class="grid gap-1 text-[15px] text-(--charcoal)">${navigation.map((item) => item[0] === 'services' ? mobileServices(page) : navLink(item, page, true)).join('')}</div>${consultationCta(cta, 'mt-3 flex items-center justify-center gap-2 rounded-full bg-(--rose-dark) px-5 py-3 text-sm font-medium text-white transition-colors')}</nav>`;
};
